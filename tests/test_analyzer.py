...
def test_swap_analysis_complete(swap_trade, eurusd_mid, fx_rates):
    ...
    # near +1M EUR and far -1M EUR: opposite-signed leg PVs...
    assert a.pv_near_leg_usd == pytest.approx(1_165_000 * DF_NEAR)
    assert a.pv_far_leg_usd == pytest.approx(-1_165_000 * DF_FAR)
    # ...whose sum is the net spot exposure (matched swap -> only the DF spread)
    assert a.spot_exposure_usd == pytest.approx(a.pv_near_leg_usd + a.pv_far_leg_usd)
...
def test_discount_factors_scale_leg_pvs(swap_trade, eurusd_mid, fx_rates):
    ...
    # spot exposure is the sum of the (discounted) leg PVs
    assert full.spot_exposure_usd == pytest.approx(
        full.pv_near_leg_usd + full.pv_far_leg_usd
    )
...
def test_valuation_date_only_affects_tenor(outright_trade, gbpusd_mid, fx_rates):
    ...
    assert a_spot.pv_near_leg_usd == pytest.approx(a_trade_date.pv_near_leg_usd)
